import React from 'react'
import './Style.css'
export default function Project() {
  return (
    <div>
      <h3 className='text-center  mt-5'>Project</h3>
      <span className='text-success'>My Projects</span>
      <div className='row m-5'>
        <div className='col-lg-4 projectpicture'>
          <img src="https://i.ibb.co/SdQ2mh4/tutorpoint-react.png" class="card-img-top 
        " alt="..." />
          <a href='https://tanuja8.github.io/Tutorpoint/'> <button type="button" className="btn btn-danger px-3 my-3">See Real Project</button></a>
        </div>
        <div className='col-lg-4 projectpicture '>
          <img src="https://i.ibb.co/R9CMZ47/Elearn.png" class="card-img-top 
        " alt="..." />
          <a href='https://tanuja8.github.io/E_learnnn/'> <button type="button" className="btn btn-danger px-3 my-3">See Real Project</button></a>
        </div>
        <div className='col-lg-4 projectpicture'>
          <img src="https://themeforest.img.customer.envatousercontent.com/files/279134394/Theme_Preview/02_Home_Page.jpg?auto=compress%2Cformat&fit=crop&crop=top&w=590&h=300&s=b527e86f633f7abee53bed4d0f364410" class="card-img-top 
        " alt="..." />
          <a href='https://tanuja8.github.io/Earboat/'><button type="button" className="btn btn-danger px-3 my-3">See Real Project</button></a>
        </div>
      </div>
      <div className='row m-5'>
      <div className='col-lg-4  projectpicture'>
          <img src="https://camo.githubusercontent.com/947f3ad49adb20e68b571d102d0677aa39209f2b81010c2043e132fb01cfd61d/68747470733a2f2f6173736574732e636362702e696e2f66726f6e74656e642f636f6e74656e742f72656163742d6a732f73746f7077617463682d6f75747075742d76322e676966" />
          <a href='https://tanuja8.github.io/stopwatch/'><button type="button" className="btn btn-danger px-3 my-3">See Real Project</button></a>
        </div>
       
        <div className='col-lg-4 projectpicture '>
          <img src="https://1.bp.blogspot.com/-4AQ6-vNhzP4/XrPKZKrZBqI/AAAAAAAAAV0/W0z04T0_Rrsf7TwMAu3jyl7eF6nrameuQCLcBGAsYHQ/s1600/Working%2BCalculator%2Busing%2BHTML%2BCSS%2Band%2BjQuery.png" />
          <a href='https://tanuja8.github.io/Calculator/'><button type="button" className="btn btn-danger px-3 my-3">See Real Project</button></a>
        </div>

        <div className='col-lg-4 projectpicture  '>
          <img src="https://themeforest.img.customer.envatousercontent.com/files/428495372/02_theme-preview.jpg?auto=compress%2Cformat&fit=crop&crop=top&w=590&h=300&s=5462b5819aaafc27dd38a5cc213e3f2a" />
          <button type="button" className="btn btn-danger px-3 my-3">See Real Project</button>
        </div>
      </div>
      <div className='row m-5'>
        <div className='col-lg-4 projectpicture'>
          <img src="https://i.ibb.co/Kw2mGq5/Screenshot-47.png" class="card-img-top 
        " alt="..." />
          <a href='https://weatherapp1411.netlify.app/'><button type="button" className="btn btn-danger px-3 my-3">See Real Project</button></a>
        </div>
        <div className='col-lg-4 projectpicture'>
          <img src="https://i.ibb.co/FgRhj78/Screenshot-49.png" class="card-img-top 
        " alt="..." />
          <a href='https://todoapp8112000.netlify.app/'><button type="button" className="btn btn-danger px-3 my-3" >See Real Project</button></a>
        </div><div className='col-lg-4 projectpicture'>
          <img src="https://i.ibb.co/pPMVFYr/shoppping.png" class="card-img-top 
        " alt="..." />
          <a href='https://product-shopping-page.netlify.app/'><button type="button" className="btn btn-danger px-3 my-3" >See Real Project</button></a>
        </div>

      </div>
    </div>
  )
}

